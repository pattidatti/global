...
type JoinGameResult =
  | { ok: true; data: { gameId: string; slotId: string } }
...
function WaxSeal({ status }: { status: ServerListEntry['status'] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 shrink-0">
      <span
        className="w-9 h-9 rounded-full shadow-seal flex items-center justify-center text-[#fbf6e9]"
        style={{
          background: `radial-gradient(circle at 30% 30%, ${meta.bg}, ${meta.ring})`,
        }}
        aria-hidden="true"
      >
        <span className="font-serif text-[10px] font-bold tracking-widest uppercase">
          {status === 'active' ? '●' : '❄'}
        </span>
      </span>
  ...
  return (
    <main className="min-h-screen bg-bg text-ink p-4 sm:p-8" aria-label="Spillservere">
      <div className="max-w-3xl mx-auto space-y-6">
        <header className="text-center pt-4 pb-6 border-b border-panelEdge/30">
          <h1
        ...
        <section className="space-y-4">
          {servers === null ? (
            ...
            <div className="parchment border border-panelEdge/60 rounded-paper shadow-paper p-8 text-center">
              <MapIcon size={36} className="mx-auto text-inkLo mb-3" strokeWidth={1.4} />
            ...
            activeServers.map(([gameId, server]) => {
              ...
              return (
                <article
                  key={gameId}
                  className="parchment border border-panelEdge/60 rounded-paper shadow-paper lift-on-hover p-5 flex items-center gap-4"
                >
